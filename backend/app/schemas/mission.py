...
class TargetDefinitionSchema(BaseModel):
    ...
    region: str = Field(default="Tamil Nadu", description="Target province / state / region")
    area: str = Field(default="Chennai Metropolitan Region", description="Target specific operational area")
    latitude: float = Field(default=13.0827, description="Target geographical latitude")
    longitude: float = Field(default=80.2707, description="Target geographical longitude")
...
class LaunchConfigurationSchema(BaseModel):
    country: str = Field(default="India", description="Launch nation")
    site: str = Field(default="Satish Dhawan Space Centre", description="Launch facility / spaceport name")
    launch_site_code: str = Field(default="SDSC_SHAR", description="Spaceport alphanumeric identifier")
...
class MissionRequestSchema(BaseModel):
    mission_name: str = Field(default="AMDSF Earth Observation Mission", description="Full descriptive mission name")
